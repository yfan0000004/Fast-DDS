...
bool Gateway::init(const std::string& config_path) {
    try {
        config_ = GatewayConfig::load(config_path);
    } catch (const std::exception& e) {
        std::cerr << "[Gateway] failed to load config: " << e.what() << std::endl;
        return false;
    }

    input_queue_.reset(new BlockingQueue<Message>(config_.queue_size));
    output_queue_.reset(new BlockingQueue<Message>(config_.queue_size));
...
void Gateway::set_filter(FilterFunc filter) {
    if (processor_) {
        processor_->set_filter(filter);
    }
}

void Gateway::set_transform(TransformFunc transform) {
    if (processor_) {
...
bool Gateway::start() {
    if (!initialized_) {
        std::cerr << "[Gateway] not initialized, call init() first" << std::endl;
        return false;
    }

    if (!router_->connect_all()) {
        std::cerr << "[Gateway] router connect failed" << std::endl;
        return false;
    }
    router_->start();
    processor_->start();
